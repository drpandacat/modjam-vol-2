...
local function tearInit(_, tear)
    ...
    if not player or player:GetPlayerType() ~= DeadlockMod.playerType.INFERNUS_B then return end
...
function afterburn:PostFireTear(tear)
    local player = DeadlockMod:TryFindPlayerSpawner(tear)
    if not player or not tear or player:GetPlayerType() ~= DeadlockMod.playerType.INFERNUS_B then return end

    local level = player:GetEffects():GetNullEffectNum(DeadlockMod.NullID.InfernusAbilities.AFTERBURN)

    if level >= 1 then
        tear:ChangeVariant(DeadlockMod.tearVariant.INFERNUS_TEAR_B)

        if level > 1 then
            tear:AddTearFlags(TearFlags.TEAR_BURN)
            local fireSprite = tear:GetTearEffectSprite()
            fireSprite.Color = DeadlockMod:ColorFromHex("#314EBF", true)
        ...
        if level == 3 then
            ...
            trail.Parent = tear
            trail.Color = DeadlockMod:ColorFromHex("#314EBF", true)
...
DeadlockMod:AddCallback(ModCallbacks.MC_POST_TEAR_UPDATE, afterburn.PostTearUpdate, DeadlockMod.tearVariant.INFERNUS_TEAR_B)

--CHECK ON THIS FUN LATER
---@param effect EntityEffect
function afterburn:PostTrailUpdate(effect)
    local tear = effect.Parent
    if not tear or tear.Variant ~= DeadlockMod.tearVariant.INFERNUS_TEAR_B then return end
...
DeadlockMod:AddCallback(ModCallbacks.MC_PRE_TEAR_COLLISION, afterburn.removeFireMind, DeadlockMod.tearVariant.INFERNUS_TEAR_B)
...
function afterburn:OnTearDeath(tear)
    ...
    splash.Color = DeadlockMod:ColorFromHex("#5168FF", true)

    --noise stuff
    DeadlockMod.sfx:Play(DeadlockMod.SoundID.InfernusAbilities.TEAR_IMPACT, volume)
end

DeadlockMod:AddCallback(ModCallbacks.MC_POST_TEAR_DEATH, afterburn.OnTearDeath, DeadlockMod.tearVariant.INFERNUS_TEAR_B)
...
local function afterburnOnDamage(_, entity, amount, flags, source, cd)
    ...
    if not player or player:GetPlayerType() ~= DeadlockMod.playerType.INFERNUS_B or flags == DamageFlag.DAMAGE_CLONES or not entity:IsActiveEnemy(false) or not entity:IsVulnerableEnemy() then --entity:IsInvincible()
        return
    end
