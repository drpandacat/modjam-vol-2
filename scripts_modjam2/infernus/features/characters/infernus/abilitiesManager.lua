local napalm = include("scripts_modjam2.infernus.features.characters.infernus.napalm")
local dash = include("scripts_modjam2.infernus.features.characters.infernus.flameDash")
local afterburn = include("scripts_modjam2.infernus.features.characters.infernus.afterburn")
local explosion = include("scripts_modjam2.infernus.features.characters.infernus.concussiveCombustion")
...
local function handleAbilities(_, player)
    if player:GetPlayerType() ~= DeadlockMod.playerType.INFERNUS then return end
...
local function renderAbilities()
    local player = PlayerManager.FirstPlayerByType(DeadlockMod.playerType.INFERNUS)
...
local function birthright(_, type, charge, firstTime, slot, varData, player)
    if player:GetPlayerType() ~= DeadlockMod.playerType.INFERNUS or not firstTime then return end
